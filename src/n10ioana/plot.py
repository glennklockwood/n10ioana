...
VAST_DBOX_NVRAM_BUFFER_GBS = VAST_DBOX_NVRAM_BUFFER_TBS * 1000
VAST_DBOX_NVRAM_BUFFER_GIBS = VAST_DBOX_NVRAM_BUFFER_GBS * 10**9 / 2**30
VAST_DBOX_NVRAM_BUFFER_TIBS = VAST_DBOX_NVRAM_BUFFER_GIBS / 1024
VAST_DBOX_QLC_CAPACITY_TBS = 15.36 * 44 * 36 / 40 # 44x 15.36 TB drives at 36+4

WORKLOAD_ID_TO_LABELS = {
    "both metadata": "Metadata",
    "write bw": "Write Bandwidth",
    "write iops": "Write IOPS",
    "read bw": "Read Bandwidth",
    "read iops": "Read IOPS",
}
...
def plot_alliops_vs_nproc_boxplot(all_results, keys=None, ax=None):
    ...
    for cidx, access in enumerate(ACCESSES):
        if keys is None:
...
def plot_alliops_vs_nproc_bynode(all_results, ax=None):
    ...
    for cidx, rw in enumerate(ACCESSES):
...
def plot_compare_rw_bw_vs_nproc(all_datasets, norm=False, access=None, ax=None):
    ...
    for idx, (key, dataset) in enumerate(all_datasets.items()):
        ...
        for jdx, jaccess in enumerate(access):
            color = 'C{:d}'.format((jdx if len(access) > 1 else idx) + 2)
            ...
            # if you just want to use a stdev
            # yerr = yerr.reindex(index=ymean.index)
            
            # if you want a confidence interval
            yerr = scipy.stats.norm.interval(
                0.68,
                loc=ymean,
                scale=yerr)# / numpy.sqrt(series.count()))
            yerr = (ymean - yerr[0], yerr[1] - ymean)
            ax.errorbar(
                x=ymean.index,
                y=ymean.values,
                yerr=yerr,
...
def plot_perf_vs_setsize(all_results, perf_key, access, relativex=False, norm_y_by=None, norm_x_by=None, ax=None):
    ...
    if norm_x_by is None:
        ...
        if relativex:
            norm_x_by = VAST_DBOX_NVRAM_BUFFER_TIBS * 1024
...
def plot_contention_losses_boxplot(dataframe, workload, workload_col="workload", color=None, xshift=0.0, relative=True, norm_x_by=None, ax=None, *args, **kwargs):
    ...
    Args:
        ...
        xshift (float): Shift the boxes by this quantity along x
        relative (bool): Plot relative loss or absolute loss.
        norm_x_by (float): If specified, divide all x values by this amount.
    ...
    if norm_x_by:
        box_labels = ["{:.2f}".format(x / norm_x_by) for x in box_labels]

    # create actual boxplot
    boxp = ax.boxplot(
        [box_data[x] for x in sorted(box_data.keys())],
        patch_artist=True,
        positions=numpy.arange(1, len(box_labels)+1) + xshift,
        labels=box_labels,
        *args,
        **boxplot_options,
    )
...
def plot_contention_losses_comparison_boxplot(datasets, workload_col, *args, **kwargs):
    ...
    num_unique = len(unique_workloads)

    fig, axes = matplotlib.pyplot.subplots(
        len(unique_workloads), 1,
        figsize=(6, 3 * num_unique),
        sharex=True)

    fig.subplots_adjust(hspace=0)

    boxwidth = 0.9 / num_unique
    patches = []
    xticks = set()
    for idx, workload in enumerate(unique_workloads):
        ...
        for jdx, (dataset_name, dataframe) in enumerate(datasets.items()):
            ...
            # draw boxplot
            xticks |= set(dataframe['primary_nodes'])
            boxp = plot_contention_losses_boxplot(
                dataframe,
                xshift=(jdx - 0.5) * boxwidth,
                widths=boxwidth,
                *args,
                **extra_args)
            if not boxp:
...
def plot_contention_losses_comparison_scatter(datasets, workload_col, styleconfigs=None, *args, **kwargs):
    ...
    for dataset in datasets.values():
        for workload in dataset[workload_col].unique():
            unique_workloads.add(workload)
        this_setup = dataset[["workload_id", 'workload']].drop_duplicates().sort_values(by="workload_id").reset_index(drop=True)
        if last_setup is not None and not last_setup.equals(this_setup):
            all_same = False
        last_setup = this_setup
    ...
    if all_same:
        workloads = last_setup.set_index("workload_id").to_dict()["workload"]
        p_workload = workloads.get("primary", "Primary")
        s_workload = workloads.get("secondary", "Secondary")
        axes[0].set_ylabel(WORKLOAD_ID_TO_LABELS.get(p_workload, p_workload))
        axes[1].set_ylabel(WORKLOAD_ID_TO_LABELS.get(s_workload, s_workload))
        axes[1].set_xlabel(axes[1].get_xlabel().replace("Primary Workload", WORKLOAD_ID_TO_LABELS.get(p_workload, p_workload)))
    else:
        for ax in axes:
...
def plot_bw_cv_vs_nproc_bars(all_results, norm=False, ax=None, fill=False):
    ...
    for aidx, access in enumerate(ACCESSES):
